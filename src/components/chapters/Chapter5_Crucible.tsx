...
const JOURNEY_LOGS: LogEntry[] = [
  { id: "L01", year: "2024", category: "Foundation", title: "Started DSA & Problem Solving", description: "Began mastering Data Structures and Algorithms, solving 200+ problems to build a strong engineering foundation." },
...
export default function Chapter5Crucible() {
  ...
  return (
    <section ref={containerRef} className="relative min-h-[150vh] bg-[#030303] py-32 overflow-hidden" id="timeline">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(var(--color-accent),0.03)_0%,transparent_70%)] pointer-events-none" />

      <div className="w-full max-w-[1200px] mx-auto px-6 md:px-12 flex flex-col gap-24 relative z-10">
        
        {/* Header */}
        <div className="flex flex-col gap-4 text-center items-center">
          <span className="text-accent font-mono text-[10px] uppercase tracking-[0.4em]">Mission Log</span>
        ...
        <div className="relative w-full max-w-4xl mx-auto">
          ...
          <div className="absolute top-0 bottom-0 left-6 md:left-1/2 -translate-x-1/2 w-[2px] bg-white/5">
            <motion.div 
              ...
              className="w-full bg-accent origin-top shadow-[0_0_15px_rgba(var(--color-accent),0.5)]" 
            />
          </div>
          ...
          <div className="flex flex-col gap-24 py-12">
            {JOURNEY_LOGS.map((log, index) => {
              ...
              return (
                ...
                >
                  {/* Timeline Node */}
                  <div className="absolute left-6 md:left-1/2 -translate-x-1/2 w-6 h-6 rounded-full bg-[#050505] border-2 border-accent flex items-center justify-center z-10 shadow-[0_0_20px_rgba(var(--color-accent),0.3)]">
                  ...
                  <div className="flex-1 pl-16 md:pl-0">
                    <div className="p-8 bg-[#0A0A0A] border border-white/10 rounded-2xl hover:border-accent/30 transition-colors shadow-2xl relative overflow-hidden group">
                      
                      {/* Watermark */}
                      <span className="absolute -bottom-4 -right-4 text-6xl font-serif font-bold text-white/[0.02] group-hover:text-accent/5 transition-colors pointer-events-none select-none">
                        {log.year}
                      </span>

                      <span className="text-accent font-mono text-[10px] uppercase tracking-[0.2em] block mb-2">
                        {log.year} {"//"} {log.category}
                      </span>
                      <h3 className="text-2xl font-serif text-white mb-4 group-hover:text-accent transition-colors">{log.title}</h3>
